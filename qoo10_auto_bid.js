...
(function () {
   ...
   // ===== 서버 시간 보정 변수 =====
   let bidEndTime = null;     // 마감 시각 (PC ms 기준, 서버 offset 보정됨)
   let serverPcOffset = 0;    // QSM 서버 시각 - PC 시각 (ms)
   ...
   // 서버 시간 기준 실제 남은 초 (탭 throttling 무관)
   function getTrueSecondsLeft() {
         if (bidEndTime === null) return getSecondsLeft();
         const estimatedServerNow = Date.now() + serverPcOffset;
         return Math.round((bidEndTime - estimatedServerNow) / 1000);
   }

   // 모니터링 시작 시 1회 서버 시간 보정
   function calibrateServerTime() {
         try {
                 const serverNow = ADBidding.server_time.getTime();
                 const pcNow = Date.now();
                 serverPcOffset = serverNow - pcNow;
                 const domSecsLeft = getSecondsLeft();
                 if (domSecsLeft !== null && domSecsLeft > 0) {
                           bidEndTime = serverNow + domSecsLeft * 1000;
                           log('⏱ 서버 시간 보정 완료 (offset: ' + (serverPcOffset >= 0 ? '+' : '') + serverPcOffset + 'ms)');
                 } else {
                           log('⚠️ 남은 시간 읽기 실패 - DOM 기반으로 대체');
                 }
         } catch (e) {
                 log('⚠️ 서버 시간 보정 실패: ' + e.message);
         }
   }
   ...
   function updateUI() {
         ...
         document.getElementById('ab-planned-price').textContent =
                 calcOptimalBid(targetRank, maxBidPrice).toLocaleString() + '¥';
         ...
         if (bids.length === 0) {
                 ...
                 listEl.innerHTML = bids.slice(0, 10).map(b => {
                           ...
                           const color = b.is_mine ? '#4CAF50' : isTarget ? '#e94560' : '#aaa';
                           const label = b.is_mine ? ' ★내것' : isTarget ? ' ◀목표' : '';
   ...
   function recordToSheet() {
         ...
         const params = new URLSearchParams({
                 ...
                 keyword: keyword,
                 my_bid: myBid ? myBid.price : myBidPrice,
                 my_rank: myBid ? myBid.rank : myBidRank,
         ...
         fetch(url + '?' + params.toString())
           .then(r => r.json())
           .then(data => {
                     if (data.status === 'ok') { log('✅ 시트 기록 완료'); setStatus('✅ 시트 기록 완료', '#4CAF50'); }
                     else { log('❌ 시트 오류: ' + (data.message || '')); }
           })
           .catch(err => log('❌ 시트 전송 실패: ' + err.message));
   }
   ...
   function executeBid() {
         ...
         if (adjustedPrice <= 0) { setStatus('❌ 입찰가 계산 오류', '#e94560'); return; }
         if (adjustedPrice > maxBidPrice) { setStatus('❌ 최대금액(' + maxBidPrice + '¥) 초과 - 입찰 취소', '#e94560'); log('최대금액 초과로 입찰 취소'); return; }
         myBidPrice = adjustedPrice;
         myBidRank = targetRank;
         log('🚀 입찰 실행! ' + adjustedPrice + '¥ (목표 ' + targetRank + '위)');
         ...
         setTimeout(() => {
                 try {
                           if (typeof ADBidding.setPlaceBidding === 'function') {
                                       ...
                                       log('✅ 입찰 제출: ' + adjustedPrice + '¥');
                                       setStatus('✅ 입찰 완료: ' + adjustedPrice.toLocaleString() + '¥', '#4CAF50');
                           } else {
                                       const btn = document.getElementById('btn_place_bid');
                                       if (btn) { btn.click(); log('✅ 버튼 클릭으로 입찰: ' + adjustedPrice + '¥'); setStatus('✅ 입찰 완료: ' + adjustedPrice.toLocaleString() + '¥', '#4CAF50'); }
   ...
   function tick() {
         ...
         if (secsLeft !== null) {
                 if (!bidFired && secsLeft <= triggerSecs && secsLeft >= 0) {
                           ...
                           executeBid();
                 } else if (secsLeft <= 0 && bidFired) {
                           setTimeout(() => { recordToSheet(); stopMonitoring(); }, 1500);
                 }
         }
   }
   ...
   function startMonitoring() {
         ...
         bidFired = false; myBidPrice = 0; myBidRank = 0;
         bidEndTime = null; serverPcOffset = 0;
         calibrateServerTime();
